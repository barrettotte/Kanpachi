...
class Ibmi():
    ...
    def __del__(self):
        self.disconnect()
        self.logger.log('IBMi connection(s) closed.')
    ...
    def login(self, creds):
        try:
            ...
            self.raise_exception("Log in unsuccessful as '{}'".format(creds['users']), e)


    def keep_alive(self):
        try:
            return self.exec_ftp(['NOOP'])
        except ftplib.all_errors as e:
            self.ftp_exception_handler('Failed to keep connection alive.', e)


    def exec_ftp(self, cmds):
        try:
            for cmd in cmds:
                self.logger.log("Executing FTP command '{}'".format(cmd))
                self.ftp_client.sendcmd(cmd)
        except ftplib.all_errors as e:
            self.ftp_exception_handler('Failed to execute command', e)
        except Exception as e2:
            self.exception_handler('Unexpected exception occurred', True, e2)
    ...
    def exception_handler(self, msg='Unexpected exception occurred', is_fatal=False, e=None):
    ...
    def raise_exception(self, msg, is_fatal=False):
        self.logger.log(msg)
        if is_fatal:
            self.disconnect()
            exit(msg)
